...
export default function Preloader({ onFinish }: PreloaderProps) {
  ...
  return (
    <div
      className={`fixed inset-0 z-100 flex items-center justify-center bg-black text-white transition-all duration-1000 ease-[cubic-bezier(0.76,0,0.24,1)] ${complete ? "-translate-y-full rounded-b-[50%]" : "translate-y-0 rounded-b-none"
        }`}
    >
      {/* Smooth Progress Bar */}
      <motion.div
      ...
      <div className={`flex ${bartle.className}`}>
        {text.split('').map((char, index) => {
          ...
          return (
            <span
              key={index}
              className={`text-[5vw] leading-none inline-block transition-all duration-300 ${isLit
